...
export class ProjectWizardStore extends BaseStore<ProjectWizardStoreState> {
	...
	goToStep = (stepIndex: number) => {
		if (stepIndex < 0 || stepIndex >= this.totalSteps) {
			logger.warn("Invalid step index", { stepIndex, totalSteps: this.totalSteps });
			return;
		}

		// Only allow navigation to completed steps or the next step
		if (stepIndex <= this.state.currentStep || this.state.completedSteps.has(stepIndex - 1)) {
	...
	goToNextStep = () => {
		if (this.canGoToNextStep) {
			...
			logger.debug("Navigated to next step", { currentStep: this.state.currentStep });
		} else {
	...
	goToPreviousStep = () => {
		if (this.canGoToPreviousStep) {
			this.state.currentStep -= 1;
			logger.debug("Navigated to previous step", { currentStep: this.state.currentStep });
		} else {
	 ...
	 */
	setStepValidation = (stepIndex: number, isValid: boolean, errors: Record<string, string> = {}) => {
	...
	validateCurrentStep = (): boolean => {
		...
		logger.debug("Validated current step", { currentStep: this.state.currentStep, isValid });
		return isValid;
	};
	...
	validateAllSteps = (): boolean => {
		const allValid = Array.from({ length: this.totalSteps }, (_, i) => i).every((stepIndex) => {
	...
	get canGoToNextStep(): boolean {
		return this.state.currentStep < this.totalSteps - 1 && this.isCurrentStepValid;
	}
	...
	get totalSteps(): number {
		...
		if (this.state.projectKind === "external" || this.state.projectKind === "student") {
			steps += 1;
		}
