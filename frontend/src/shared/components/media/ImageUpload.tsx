...
} from "@/shared/utils/image-compression.utils";
	...
	const handleFileSelect = async (file: File | null) => {
		...
		if (fileSizeMB > 5) {
			loadingToast = toast.loading(
				`Compressing ${fileSizeMB.toFixed(1)}MB image...`
			);
		}
	...
	const renderPreview = () => {
		...
		if (isLoading) {
			...
			return (
				<div
					className={cn(
						"relative overflow-hidden rounded-lg flex items-center justify-center bg-muted",
						config.defaultSize
					)}
				>
