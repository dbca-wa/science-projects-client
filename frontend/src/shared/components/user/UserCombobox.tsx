...
import { getUsersBasedOnSearchTerm, getFullUser } from "@/features/users/services/user.service";
	...
	(
		...
		return (
			<BaseCombobox<IUserData>
				...
				renderMenuItem={(user: IUserData, onSelect: (user: IUserData) => void, isHighlighted: boolean) => (
					<UserMenuItem user={user} onSelect={onSelect} isHighlighted={isHighlighted} />
				)}
				icon={showIcon ? <User className="size-4 text-gray-500 dark:text-gray-400" /> : undefined}
				showIcon={showIcon}
...
const SelectedUserDisplay = ({ user, onClear }: SelectedUserDisplayProps) => {
	...
	return (
		<div className={cn("flex items-center relative px-2 py-1.5 rounded-md bg-gray-100 dark:bg-gray-700 h-11")}>
			...
			>
				{getUserDisplayName(user)} {user.is_staff ? (user.is_superuser ? "(Admin)" : "(Staff)") : "(External)"}
			</span>
		...
		>
			...
			<div className="flex items-center justify-start ml-3 h-full">
				...
				>
					...
					{user.is_staff ? (user.is_superuser ? "(Admin)" : "(Staff)") : "(External)"}
				</span>
			</div>
		</button>
	);
