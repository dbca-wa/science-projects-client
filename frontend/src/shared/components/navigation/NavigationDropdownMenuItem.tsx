...
interface NavigationDropdownMenuItemProps extends Omit<ComponentProps<typeof DropdownMenuItem>, 'onClick'> {
  targetPath: string;
 ...
 * </DropdownMenu>
 */
export const NavigationDropdownMenuItem = forwardRef<HTMLAnchorElement, NavigationDropdownMenuItemProps>(
  ({ targetPath, onNavigate, children, className, ...props }, ref) => {
    ...
    return (
      <DropdownMenuItem 
        className={cn(
          className,
          isActive && "bg-blue-100 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/30"
        )} 
        asChild 
        {...props}
      >
