...
function buildBreadcrumbTrail(
	...
	if (currentRoute.breadcrumbParent) {
		const parentRoute = ALL_FLATTENED_ROUTES.find(
			(r) => r.path === currentRoute.breadcrumbParent
		);
		if (parentRoute) {
			breadcrumbs.push({
...
function findMatchingRoute(pathname: string): RouteConfig | undefined {
	...
	const route = ALL_FLATTENED_ROUTES.find((r) => r.path === pathname);
