...
export const useSearchStoreInit = <TFilters = Record<string, unknown>>({
	...
	useEffect(() => {
		...
		if (shouldLoadFromStorage && savedData) {
			...
			const businessAreas =
				filtersData.selectedBusinessAreas || data.selectedBusinessAreas;
			if (
				businessAreas &&
			...
			const userFilter =
				filtersData.user !== undefined ? filtersData.user : data.filterUser;
			...
			const kindFilter =
				filtersData.kind !== undefined ? filtersData.kind : data.filterKind;
			...
			const yearFilter =
				filtersData.year !== undefined ? filtersData.year : data.filterYear;
			...
			const onlyActive =
				filtersData.onlyActive !== undefined
					? filtersData.onlyActive
					: data.onlyActive;
			...
			const onlyInactive =
				filtersData.onlyInactive !== undefined
					? filtersData.onlyInactive
					: data.onlyInactive;
