...
import {
	...
	isProjectLeader,
	isCaretakerOfProjectLeader,
	type IDocumentData,
} from "@/shared/utils/project-permissions.utils";
...
describe("isCaretakerOfProjectLeader", () => {
	it("should return true when user is caretaker of project leader", () => {
		const user = {
			...createMockUser({ id: 2 }),
			caretaking_for: [
				{
					id: 1,
					display_first_name: "Leader",
					display_last_name: "User",
		...
		expect(isCaretakerOfProjectLeader(user, members)).toBe(true);
	});

	it("should return false when user is not caretaker of project leader", () => {
		const user = {
			...createMockUser({ id: 2 }),
			caretaking_for: [
				{
		...
		expect(isCaretakerOfProjectLeader(user, members)).toBe(false);
	});

	it("should return false when user has no caretaking_for field", () => {
		const user = createMockUser({ id: 2 });
		const members = [
			createMockMember({
				is_leader: true,
	...
	it("should return false when user is null", () => {
		const members = [createMockMember({ is_leader: true })];
		expect(isCaretakerOfProjectLeader(null, members)).toBe(false);
	});
});

describe("canEditProject with caretaker permissions", () => {
	it("should grant permission to caretaker of admin", () => {
		const user = {
			...createMockUser({ id: 2, is_superuser: false }),
			caretaking_for: [
				{
					id: 1,
	...
	it("should grant permission to caretaker of project leader", () => {
		const user = {
			...createMockUser({ id: 2, is_superuser: false }),
			caretaking_for: [
				{
	...
	it("should grant permission to caretaker of business area leader", () => {
		const user = {
			...createMockUser({ id: 3, is_superuser: false }),
			caretaking_for: [
				{
					id: 2,
					display_first_name: "BA Leader",
					display_last_name: "User",
					email: "baleader@example.com",
					image: "",
					end_date: null,
					is_superuser: false,
					caretakers: [],
	...
	it("should grant permission to project leader", () => {
		const user = createMockUser({ id: 1, is_superuser: false });
		const project = createMockProject({
			team_members: [
				createMockMember({
					is_leader: true,
