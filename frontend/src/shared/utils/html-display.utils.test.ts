...
describe("html-display.utils", () => {
	describe("styleHtmlForDisplay", () => {
		...
		it("should style links with Tailwind classes", () => {
			...
			const result = styleHtmlForDisplay(html);
			expect(result).toContain('class="text-blue-600 dark:text-blue-400 underline');
		...
		it("should handle mixed content", () => {
			const html = '<p><b>Bold</b> and <a href="https://example.com" style="color: red;">link</a></p>';
			const result = styleHtmlForDisplay(html);
			expect(result).not.toContain("<b>");
	...
	describe("isEmptyRichTextContent", () => {
		...
		it("should return true for light editor empty paragraph", () => {
			expect(isEmptyRichTextContent('<p class="editor-p-light"><br></p>')).toBe(true);
		});

		it("should return true for dark editor empty paragraph", () => {
			expect(isEmptyRichTextContent('<p class="editor-p-dark"><br></p>')).toBe(true);
		});
		...
		it("should return false for content with formatted text", () => {
			expect(isEmptyRichTextContent("<p><strong>Bold</strong></p>")).toBe(false);
		});
	});
