...
import { ACCEPTED_IMAGE_TYPES, MAX_IMAGE_SIZE_MB } from "@/shared/constants/image.constants";
...
describe("validateImageType", () => {
	...
	it("should reject when not in custom accepted types", () => {
		...
		expect(() => validateImageType(file, customTypes)).toThrow(ImageCompressionError);
	});
});
...
describe("compressImage", () => {
	...
	describe("validation", () => {
		it("should validate file type before compression", async () => {
			...
			await expect(compressImage(invalidFile)).rejects.toThrow(ImageCompressionError);
			await expect(compressImage(invalidFile)).rejects.toThrow("Invalid file type");
		});
	...
	describe("large files (compression needed)", () => {
		...
		it("should throw COMPRESSION_FAILED when compression fails", async () => {
			...
			try {
				...
				expect(error).toBeInstanceOf(ImageCompressionError);
				expect((error as ImageCompressionError).code).toBe("COMPRESSION_FAILED");
				expect((error as ImageCompressionError).message).toContain("Failed to compress");
			}
		});
	});
	...
	describe("custom options", () => {
		...
		it("should use custom acceptedTypes", async () => {
			const pngFile = createMockFile({ type: "image/png", size: 2 * 1024 * 1024 });
