...
export const EndorsementTasksDataTable = ({ tasks, kind }: EndorsementTasksDataTableProps) => {
	...
	const columns: ColumnDef<IEndorsement>[] = useMemo(
		() => [
			...
			},
			{
				id: "project",
				header: "Project",
				accessor: (row) => extractTextFromHTML(row.project_plan.document.project.title),
				cell: (row) => {
					const projectTitle = extractTextFromHTML(row.project_plan.document.project.title);
					return (
						<div>
	...
	const handleRowClick = (endorsement: IEndorsement, event: React.MouseEvent) => {
