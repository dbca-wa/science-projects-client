...
import { isValidCaretaking, hasNestedCaretakee } from "../utils";
import type {
	IProjectMember,
	IProjectData,
...
export const useCaretakerPermissions = () => {
	 ...
	 */
	const canActForUser = useCallback(
		(targetUserId: number): boolean => {
			if (!user?.caretaking_for) return false;

			// Check direct caretaking
			const isDirectCaretaker = user.caretaking_for.some(
				(caretakee) =>
					caretakee.id === targetUserId && isValidCaretaking(caretakee)
			);
	 ...
	 */
	const canActAsProjectMember = useCallback(
		(members: IProjectMember[]): boolean => {
			if (!members || members.length === 0) return false;

			return members.some((member) => canActForUser(member.user.id));
		},
		[canActForUser]
	);
	 ...
	 */
	const canActAsProjectLead = useCallback(
		(project: IProjectWithMembers): boolean => {
			if (!project?.members) return false;

			return project.members.some(
				(member: IProjectMember) =>
					member.is_leader && canActForUser(member.user.id)
			);
		},
		[canActForUser]
	);
	 ...
	 */
	const canActAsBusinessAreaLead = useCallback(
