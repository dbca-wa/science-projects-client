...
function calculateCentroid(feature: GeoJSON.Feature): [number, number] | null {
  try {
    ...
    console.error("Error calculating centroid:", error instanceof Error ? error.message : String(error));
    return null;
  }
}
  ...
  const handleClick = () => {
    if (onLabelClick) {
      try {
        ...
        console.error("Error getting bounds for label click:", error instanceof Error ? error.message : String(error));
      }
    }
  };
