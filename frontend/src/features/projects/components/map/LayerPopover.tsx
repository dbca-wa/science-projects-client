...
const LAYER_DESCRIPTIONS: Record<StoreLayerType, string> = {
	...
	imcra:
		"Integrated Marine and Coastal Regionalisation of Australia marine regions",
};
...
export const LayerPopover = observer(
	({ isFullscreen: _isFullscreen = false }: LayerPopoverProps) => {
		...
		useEffect(() => {
			...
			const popoverContent =
				popoverRef.current?.querySelector('[role="dialog"]');
			if (popoverContent) {
		...
		const handleKeyDown = (event: React.KeyboardEvent) => {
			...
			if (event.key === "Escape") {
				...
				const trigger = popoverRef.current?.querySelector(
					'[data-state="open"]'
				);
