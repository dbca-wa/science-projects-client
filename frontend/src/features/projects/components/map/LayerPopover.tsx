 ...
 * Only DBCA layers are active; NRM/IBRA/IMCRA disabled pending GeoJSON data.
 */
type StoreLayerType = "dbcaregion" | "dbcadistrict";
// | "nrm" | "ibra" | "imcra";
...
const LAYER_DESCRIPTIONS: Record<StoreLayerType, string> = {
	dbcaregion:
		"Department of Biodiversity, Conservation and Attractions administrative regions",
 ...
 */
export const LayerPopover = observer(function LayerPopover({
	...
	useEffect(() => {
		const handleKeyDown = (event: KeyboardEvent) => {
			if (event.key === "Escape") {
				...
				const popoverTrigger = popoverRef.current?.querySelector(
					'[data-state="open"]'
				);
	...
	const allLayersVisible =
		store.state.visibleLayerTypes.length === Object.keys(LAYER_NAMES).length;
			...
			<DropdownMenu>
				<DropdownMenuTrigger asChild>
					<Button
						...
						aria-label="Adjust Layers - toggle map layer visibility and display options"
						title="Adjust Layers"
					>
					...
					>
						<div className="space-y-4">
							...
							<div className="space-y-3">
								{(Object.keys(LAYER_NAMES) as StoreLayerType[]).map(
									(layerType) => {
										const isVisible =
											store.state.visibleLayerTypes.includes(layerType);
							...
							<div className="border-t border-gray-200 dark:border-gray-700 pt-3">
								<p className="text-xs text-gray-500 dark:text-gray-400">
									{store.state.visibleLayerTypes.length} of{" "}
									{Object.keys(LAYER_NAMES).length} layers visible
								</p>
							</div>
						</div>
					</div>
