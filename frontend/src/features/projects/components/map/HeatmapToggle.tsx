...
export const HeatmapToggle = observer(() => {
	...
	const handleToggle = () => {
		...
		setTimeout(() => {
			if (wasHeatmapMode) {
				announceToScreenReader("Switched to marker view. Individual project markers are now displayed.");
			} else {
				announceToScreenReader("Switched to heatmap view. Project density visualization is now displayed.");
			}
		}, 100);
	};
	...
	return (
		<Button
			...
			aria-label={store.isHeatmapMode ? "Switch to marker view" : "Switch to heatmap view"}
			title={store.isHeatmapMode ? "Switch to marker view" : "Switch to heatmap view"}
		>
			{store.isHeatmapMode ? (
				<>
