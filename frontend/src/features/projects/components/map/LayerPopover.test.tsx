...
describe("LayerPopover", () => {
	...
	it("should disable Show All when all layers are visible", async () => {
		mockStore.state.visibleLayerTypes = ["dbcaregion", "dbcadistrict", "nrm", "ibra", "imcra"];
	...
	it("should render layer descriptions", async () => {
		...
		await waitFor(() => {
			expect(screen.getByText(/Department of Biodiversity, Conservation and Attractions/)).toBeInTheDocument();
			expect(screen.getByText(/Natural Resource Management regions/)).toBeInTheDocument();
		});
	});
	...
	it("should have proper accessibility attributes", async () => {
		...
		await waitFor(() => {
			const dbcaRegionsCheckbox = screen.getByLabelText("DBCA Regions");
			expect(dbcaRegionsCheckbox).toHaveAttribute("aria-describedby", "layer-dbcaregion-desc");
			
			const labelsSwitch = screen.getByLabelText("Show Labels");
			expect(labelsSwitch).toHaveAttribute("aria-describedby", "show-labels-desc");
		});
	});
});