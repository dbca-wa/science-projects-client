...
export const createProjectMarker = (
	...
	const sizeConfig = useDynamicSize
		? getMarkerSize(count)
		: { size: 40, iconAnchor: [20, 20] };
	...
	const sizeStyle = useDynamicSize
		? `width: ${sizeConfig.size}px; height: ${sizeConfig.size}px;`
		: "";
	...
	const icon = L.divIcon({
      ...
      <div class="relative group cursor-pointer ${zIndexClass}" role="button" aria-label="${safeAriaLabel}" tabindex="0">
        <div class="${useDynamicSize ? "" : "w-10 h-10"} rounded-full shadow-lg transition-transform ${scaleClass} focus:outline-none focus:ring-2 ${ringClass}"
             style="background-color: ${color}${useDynamicSize ? "; " + sizeStyle : ""}">
    ...
    `,
		...
		popupAnchor: useDynamicSize
			? [0, -sizeConfig.iconAnchor[1] + 10]
			: [0, -20],
	});
