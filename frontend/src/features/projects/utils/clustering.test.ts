	...
	describe("clusterProjects", () => {
		...
		it("should create separate clusters for distant projects", () => {
			const projects = [
				...
				createProjectWithCoords(2, [30.0, 40.0]),
				createProjectWithCoords(3, [50.0, 60.0]),
			];
			const result = clusterProjects(projects);
		...
		it("should handle mix of clustered and separate projects", () => {
			const projects = [
				...
				createProjectWithCoords(3, [30.0, 40.0]), // Separate
				createProjectWithCoords(4, [30.0, 40.0]), // Cluster with 3
				createProjectWithCoords(5, [50.0, 60.0]), // Separate
			];
			const result = clusterProjects(projects);
		...
		it("should handle coordinates near equator and prime meridian", () => {
			const projects = [
				createProjectWithCoords(1, [0.0, 0.0]),
				createProjectWithCoords(2, [0.0, 0.0]),
			];
			const result = clusterProjects(projects);

			expect(result.size).toBe(1);
			const cluster = Array.from(result.values())[0];
			expect(cluster.coords).toEqual([0.0, 0.0]);
		});
		...
		it("should use first project coordinates as cluster coordinates", () => {
			...
			const cluster = Array.from(result.values())[0];
			expect(cluster.coords).toEqual([10.1234, 20.5678]); // Uses first project's coords
		});
