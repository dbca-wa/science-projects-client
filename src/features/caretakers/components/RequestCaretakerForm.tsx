...
import { useRequestCaretaker } from "../hooks/useRequestCaretaker";
...
export const RequestCaretakerForm = ({
	...
	const ignoreArray = useMemo(() => {
		...
		caretakingChainIds,
		caretakerData?.become_caretaker_request_object?.primary_user?.id,
	]);
	...
	return (
		<>
			<div className="space-y-6">
				<div>
					<h3 className="text-lg font-semibold mb-2">Request Caretaker</h3>
					<p className="text-sm text-muted-foreground mb-4">
						Request someone to manage your projects during your absence. All
						caretaker requests require admin approval.
					</p>
				</div>
				...
				<Form {...form}>
					...
					>
							...
							render={({ field }) => (
								<FormItem>
									...
									<p className="text-sm text-muted-foreground mt-2">
										Users you are caretaking for (and their caretakees) cannot
										be selected to prevent circular chains. Additionally, users
										who already have a caretaker cannot be selected.
									</p>
									<FormMessage />
								</FormItem>
							)}
								...
								<AlertDescription>
									...
									{requestCaretakerMutation.error?.message?.includes(
										...
										<p className="mt-2 text-sm">
											The selected user already has a caretaker assigned to them
											and cannot be a caretaker for others. Please select a
											different user.
										</p>
									)}
								</AlertDescription>
						...
						<Button
							type="submit"
							disabled={!isFormValid || requestCaretakerMutation.isPending}
							className="w-full"
						>
			...
			<AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
				<AlertDialogContent>
					<AlertDialogHeader>
						<AlertDialogTitle>Confirm Caretaker Request</AlertDialogTitle>
						<AlertDialogDescription>
							...
							{isAdmin && approveAsAdmin ? (
								...
								<span className="block mt-2">
									This will create a pending request that requires admin
									approval.
								</span>
							)}
						</AlertDialogDescription>
