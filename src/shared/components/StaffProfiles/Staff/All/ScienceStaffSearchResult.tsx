...
const ScienceStaffSearchResult = ({
  ...
  return (
    <div className="relative min-h-48 rounded-md border border-blue-300 p-6">
      {is_hidden && (
        <div className="absolute top-0 right-0 rounded-tr-md rounded-bl-md bg-red-500 px-2 py-1 text-xs text-white">
          <TooltipProvider>
            <Tooltip>
      ...
      <h2
        className="cursor-pointer text-lg font-bold text-balance text-blue-600 hover:text-blue-500 hover:underline"
        onClick={() => {
          const staffId = pk;
      ...
      <div className="my-3">
        {position && (
          <span className="flex items-center">
            ...
            <p className="ml-2 flex-1 text-sm text-balance">{position}</p>
          </span>
        )}
        {division && (
          <span className="flex items-center">
            <Building className="mt-[2px] self-start" size={"15px"} />
            <p className="ml-2 flex-1 text-sm text-balance">
              {division}. {unit}
            </p>
          </span>
        )}
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="p-4 text-slate-800 sm:max-w-[425px]">
        ...
        <DialogHeader>
          <DialogTitle className="mt-3 mb-2">Email {`${name}`}</DialogTitle>
        </DialogHeader>
    ...
    <Drawer open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DrawerContent className="p-4">
        ...
        <div className="mx-auto w-full max-w-sm text-slate-800">
          <DrawerHeader>
            <DrawerTitle className="mt-3 mb-2">Email {`${name}`}</DrawerTitle>
          </DrawerHeader>
          <EmailStaffMemberContent
            kind={"drawer"}
            pk={pk}
