...
const LayoutSwitcherContext = createContext<ILayoutContext>({
  layout: "traditional",
  switchLayout: () => {
    throw new Error("switchLayout function must be overridden");
  },
  loading: false,
  setLoading: () => {
...
export const LayoutSwitcherProvider = ({
  ...
  const switchLayout = () => {
    ...
    setLayout((prevLayout) =>
      prevLayout === "modern" ? "traditional" : "modern"
    );
  };

  return (
    <LayoutSwitcherContext.Provider
      value={{ layout, switchLayout, loading, setLoading }}
    >
      {children}
    </LayoutSwitcherContext.Provider>
  );
