...
const MyProfilePage = observer(() => {
	...
	const { data: outgoingRequests } = useOutgoingCaretakerRequests(
		user?.id || 0
	);
	...
	const [isToggleVisibilityModalOpen, setIsToggleVisibilityModalOpen] =
		useState(false);
		...
		if (hasPendingRequests) {
			return (
				<Alert>
					...
					<AlertDescription>
						You have a pending caretaker request. Please wait for it to be
						approved or cancel it before requesting a new caretaker.
					</AlertDescription>
				</Alert>
			);
		}
						...
						<TabsContent value="staff-profile" className="space-y-6">
							<div className="max-w-4xl mx-auto space-y-6">
								...
								<div className="text-sm text-muted-foreground text-center pt-4 border-t">
									<p>
										Your staff profile is{" "}
										{user.staff_profile_hidden ? "hidden" : "visible"} to the
										public.
					...
					<TabsContent value="caretaker" className="space-y-6">
						<div className="max-w-4xl mx-auto">
							...
							<Alert className="mb-6">
								<Info className="h-4 w-4" />
								<AlertDescription>
									Caretakers can manage your projects when you're unavailable
									(on leave, resignation, or other absence). All caretaker
									requests require admin approval for security and
									accountability.
								</AlertDescription>
							</Alert>
							...
							{isLoadingCaretaker ? (
								...
								<Alert variant="destructive">
									...
									<AlertDescription>
										Failed to load caretaker information. Please try again
										later.
									</AlertDescription>
								</Alert>
							) : (
