...
export const WordCount = ({ text, wordLimit, limitCanBePassed, setCanSave }: IWordCountProps) => {

    const [charLimit, setCharLimit] = useState<number>(wordLimit ? wordLimit * 5 : 0);
    const [limitAlmostExceeded, setLimitAlmostExceeded] = useState<boolean>(false);
            ...
            {wordLimit ?
                limitExceeded === true ? (
                    ...
                    <Text
                        ...
                        fontWeight={"bold"}
                    >
                        {`${limitCanBePassed ? "Aim for max of" : "Limit"}: ${wordLimit} words `}
                    </Text>
