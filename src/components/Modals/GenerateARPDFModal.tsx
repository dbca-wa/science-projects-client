...
export const GenerateARPDFModal = ({
    isOpen,
    onClose,
    thisReport,
}: Props) => {
    ...
    const generatePDFMutation = useMutation(generateReportPDF, {
        onMutate: () => {
                            ...
                            >
                                <Box px={4} mb={4}>
                                    <Text fontWeight={"semibold"} fontSize={"xl"}>
                                        You are about to generate a pdf{thisReport?.year && ` for Annual Report FY ${thisReport.year}-${thisReport.year - 1}`}.                                    </Text>
                                </Box>
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        {Object.entries(sectionDictionary).map(([key, value], index) => (
                                            <option key={key} value={key}>{index + 1}. {value}</option>
                                        ))}

                                    </Select>
                                    <FormHelperText>
                                        Select the section of the annual report you would like to generate
                                    </FormHelperText>
                                </FormControl>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {baData?.map((ba: IBusinessArea) => (
                                                <option key={ba.name} value={Number(ba?.pk)}>{ba.name}</option>
                                            ))}

                                        </Select>
                                        <FormHelperText>
                                            This is optional, but may save time. By default, all business areas will be included.
                                        </FormHelperText>
                                    </FormControl>
                                )}

                                <Text fontWeight={"semibold"} color={"red.500"} mt={8} mb={12} px={80}>
                                    Note: Further PDF generations for this report will be disabled until this operation has completed.
                                </Text>
                    ...
                    <ModalFooter>
                        <Grid gridTemplateColumns={"repeat(2, 1fr)"} gridGap={4}>
                            ...
                            <Button
                                ...
                                type="submit"
                                isDisabled={generatePDFMutation.isLoading || isGenerateReportButtonDisabled || !selectedSection}
                                ml={3}
                                leftIcon={<FaFilePdf />}
                            >
                                Generate PDF
