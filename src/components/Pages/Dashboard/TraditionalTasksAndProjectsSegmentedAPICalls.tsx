...
export const TraditionalTasksAndProjectsSegmentedAPICalls = ({ onAddTaskOpen }: Props) => {
    ...
    // const { pendingProjectDocumentData, pendingProjectDocumentDataLoading } = useGetDocumentsPendingMyAction(); // Gets all at once (slower)
    // useEffect(() => {
    //     console.log(pendingProjectDocumentData)
    // }, [pendingProjectDocumentData])
    // Separated for speed:
    const { docsPendingStageOneInput, docsPendingStageOneInputLoading } = useGetDocumentsPendingStageOneInput();
    const { docsPendingStageTwoInput, docsPendingStageTwoInputLoading } = useGetDocumentsPendingStageTwoInput();
    const { docsPendingStageThreeInput, docsPendingStageThreeInputLoading } = useGetDocumentsPendingStageThreeInput();

    useEffect(() => {
        if (!docsPendingStageThreeInputLoading && !docsPendingStageTwoInputLoading && !docsPendingStageOneInputLoading) {
            console.log({
    ...
    }, [docsPendingStageOneInput, docsPendingStageOneInputLoading, docsPendingStageTwoInputLoading, docsPendingStageTwoInput, docsPendingStageThreeInputLoading, docsPendingStageThreeInput])
        ...
        <>
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        >
                            ...
                            </Box>
                            {
                                (
                                    (
                                        (!tasksLoading && !docsPendingStageOneInputLoading && !docsPendingStageTwoInputLoading && !docsPendingStageThreeInputLoading) &&
                                        (combinedData?.length +
                        ...
                        >
                            ...
                            >
                                {(combinedData?.length >= 1) && combinedData.map((task: ITaskDisplayCard, index: number) => {
                                    return (
...

                                {
                                    ...
                                    (docsPendingStageOneInputLoading === true)
                                        ...
                                        (docsPendingStageOneInput?.team?.length + docsPendingStageOneInput?.lead?.length >= 1)
                                            ?
                                            (
                                                (
                                                    [
                                                        ...docsPendingStageOneInput.team.map((document) => ({ document, inputKind: 'team_member' })),
                                                        ...docsPendingStageOneInput.lead.map((document) => ({ document, inputKind: 'project_lead' })),
                                                    ]
                                ...
                                }
                                {
                                    ...
                                    (docsPendingStageTwoInputLoading === true)
                                        ...
                                        (docsPendingStageTwoInput?.length >= 1)
                                            ?
                                            (
                                                docsPendingStageTwoInput[0]?.map((document, index: number) => {
                                ...
                                }
                                {
                                    ...
                                    (docsPendingStageThreeInputLoading === true)
                                        ...
                                        (docsPendingStageThreeInput?.length >= 1)
                                            ?
                                            (
                                                docsPendingStageThreeInput[0]?.map((document, index: number) => {
                                                    console.log(document)
                                                    return (
                                                        <TraditionalDocumentTaskDisplay
                                                            key={index}
                    ...
                    {
                        (me?.userData?.is_aec || me?.userData?.is_biometrician || me?.userData?.is_herbarium_curator || me?.userData?.is_superuser)
                            ?
                            (
                                <AccordionItem
                                    borderColor={colorMode === "light" ? "blackAlpha.500" : "whiteAlpha.600"}
