		...
		if (!tasksLoading && taskData) {
			...
			const sortTasksByStatus = (tasks) => {
				return tasks.sort((a, b) => {
					...
					if (a.status === "inprogress" && b.status !== "done")
						return -1;
					return 0;
				});
			};
				...
				>
					{pendingProjectDocumentDataLoading ? (
						...
						<motion.div
							...
							animate={{
								opacity: pendingProjectDocumentDataLoading
									? 0
									: 1,
							}}
						...
						>
							...
							>
								...
								>
									...
									{/* </Box> */}
									{combinedData?.length +
										pendingProjectDocumentData?.all
											?.length >=
									1 ? (
										<Box
											display={"inline-flex"}
								...
								>
									...
									1 ? (
										...
										>
											...
											{!pendingProjectDocumentDataLoading &&
											pendingProjectDocumentData?.team
												?.length +
												pendingProjectDocumentData?.lead
													?.length +
												pendingProjectDocumentData?.ba
													?.length +
												pendingProjectDocumentData
													?.directorate?.length >=
												1
						...
						>
							...
							>
								{projectData?.length >= 1 ? (
									...
									<Text mt={4} mx={2}>
										You are currently not associated with
										any projects.
									</Text>
								)}
							</AccordionPanel>
