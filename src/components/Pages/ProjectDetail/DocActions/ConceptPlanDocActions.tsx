...
import {
    ...
    setProjectStatus, ISetProjectProps
} from "../../../../lib/api";
...
export const ConceptPlanDocActions = ({ all_documents, conceptPlanData, refetchData,
    // , projectPk 
    ...
    const { teamData, isTeamLoading, refetchTeamData } = useProjectTeam(String(conceptPlanData?.document?.project?.pk));
    ...
    const spawnMutation = useMutation(spawnNewEmptyDocument,
        {
            ...
            onSuccess: async (data) => {
                ...
                const updateData: ISetProjectProps = {
                    projectId: conceptPlanData?.document?.project.pk ? conceptPlanData.document.project.pk : conceptPlanData?.document?.project?.id,
                    status: 'pending',
                }
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <Tag
                                            bg={
                                                conceptPlanData.document.status === "approved" ? "green.500" :
                                                    conceptPlanData.document.status === "inapproval" ? "blue.500" :
                                                        conceptPlanData.document.status === "inreview" ? "orange.500" :
                                                            conceptPlanData.document.status === "revising" ? "orange.500" :
                                                                // New
                                                                "red.500"
                                            }
                                            color={"white"}
                                        ...
                                        >
                                            {
                                                conceptPlanData.document.status === "inapproval" ?
                                                    "Approval Requested" :
                                                    conceptPlanData.document.status === "approved" ? "Approved" :
                                                        conceptPlanData.document.status === "inreview" ? "Review Requested" :
                                                            conceptPlanData.document.status === "revising" ? "Revising" : "New Document"
                                            }
                                        </Tag>
                                    </Flex>
                                    <Flex
                                    ...
                                    >
                                        ...
                                        >
                                            Document ID</Text>
                                        <Text>
                                            {conceptPlanData?.document?.pk ? conceptPlanData?.document?.pk : conceptPlanData?.document?.id}
                                        </Text>
                                    </Flex>
                                </Grid>
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Tag
                                                ...
                                                display={"flex"}
                                                bg={conceptPlanData?.document?.project_lead_approval_granted === true ? "green.500" : "red.500"}
                                                color={"white"}
                                            >
                                                {conceptPlanData?.document?.project_lead_approval_granted === true ? "Granted" : "Required"}
                                            </Tag>
                                        ...
                                        <Grid
                                            justifyContent={"flex-end"}
                                            w={"100%"}
                                            mt={conceptPlanData?.document?.business_area_lead_approval_granted ? 0 : 3}
                                            gridTemplateColumns={"repeat(1, 1fr)"}
                                        >
                                            {conceptPlanData?.document?.business_area_lead_approval_granted === false
                                                && conceptPlanData?.document?.project_lead_approval_granted === true
                                                && (userData?.is_superuser || userData?.pk === leaderMember?.user?.pk) && (
                                                    ...
                                                    >
                                                        <ConceptPlanActionModal
                                                            ...
                                                            documentPk={conceptPlanData?.document?.pk ? conceptPlanData?.document?.pk : conceptPlanData?.document?.id}
                                                            stage={1}
                                            ...
                                            {conceptPlanData?.document?.business_area_lead_approval_granted === false &&
                                                (conceptPlanData?.document?.id || conceptPlanData?.document?.pk) &&
                                                conceptPlanData?.document?.project_lead_approval_granted === false
                                                && (userData?.is_superuser || userData?.pk === leaderMember?.user?.pk) && (
                                                    <Center
                                                        justifyContent={"flex-end"}>
                                                        <ConceptPlanActionModal
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Tag
                                                ...
                                                justifyContent={"center"}
                                                display={"flex"}
                                                bg={conceptPlanData?.document?.business_area_lead_approval_granted === true ? "green.500" : "red.500"}
                                                color={"white"}
                                            >
                                                {conceptPlanData?.document?.business_area_lead_approval_granted === true ? "Granted" : "Required"}
                                            </Tag>
                                        ...
                                        <Flex
                                            justifyContent={"flex-end"}
                                            w={"100%"}
                                            mt={conceptPlanData?.document?.project_lead_approval_granted && conceptPlanData?.document?.directorate_approval_granted === false ? 3 : 0}
                                        // gridTemplateColumns={"repeat(2, 1fr)"}
                                        >
                                            ...
                                            {conceptPlanData?.document?.business_area_lead_approval_granted
                                                && conceptPlanData?.document?.directorate_approval_granted === false
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <Tag
                                                ...
                                                justifyContent={"center"}
                                                display={"flex"}
                                                bg={conceptPlanData?.document?.directorate_approval_granted === true ? "green.500" : "red.500"}
                                                color={"white"}
                                            >
                                                {conceptPlanData?.document?.directorate_approval_granted === true ? "Granted" : "Required"}
                                            </Tag>
                                        ...
                                        <Flex
                                            justifyContent={"flex-end"}
                                            w={"100%"}
                                            mt={conceptPlanData?.document?.business_area_lead_approval_granted ? 3 : 0}
                                        >


                                            {conceptPlanData?.document?.business_area_lead_approval_granted && conceptPlanData?.document?.directorate_approval_granted === false
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                <Input
                                                    type="hidden"
                                                    {...register("docPk", { required: true, value: conceptPlanData?.document?.pk })}
                                                />
                                                <Input
                                                    type="hidden"
                                                    {...register("kind", { required: true, value: conceptPlanData?.document?.kind })}
                                                />
                                            </Box>
                                            <Box
                                                as="form"
