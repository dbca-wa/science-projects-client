...
import { Box, Button, Flex, FormControl, FormHelperText, FormLabel, Grid, Icon, Input, InputGroup, InputLeftElement, Spinner, Text, useColorMode, useToast } from "@chakra-ui/react";
import { Head } from "../components/Base/Head";
import { GrMail } from "react-icons/gr";
import { createUser, getDoesUserWithEmailExist, getDoesUserWithFullNameExist } from "../lib/api";
...
export const CreateUser = ({ onSuccess }: { onSuccess?: () => void }) => {
    ...
    useEffect(() => {
        if (isFirstNameValid && isLastNameValid && !lastNameError && !firstNameError) {
    ...
    const delayedNameCheck = async (firstName: string, lastName: string) => {
        try {
            const doesNameExist = await getDoesUserWithFullNameExist({ firstName, lastName });
            ...
            <form onSubmit={handleSubmit}>
                ...
                >
                    ...
                    <FormControl isRequired my={2} mb={4}>
                        ...
                        {nameExists && (
                            <FormHelperText color="red.500">User with this name already exists.</FormHelperText>
                        )}
                        {firstNameError && (
                ...
                >
                    ...
                    <FormControl isRequired my={2} mb={4}>
                        ...
                        {isCheckingEmail && (
                            <FormHelperText color="blue.500">Checking email...</FormHelperText>
                        )}
                        {emailExists && (
                            <FormHelperText color="red.500">
                        ...
                        {
                            !emailExists && email.length >= 5 && isValidEmail && emailsMatch &&
                            firstName.length > 1 && lastName.length > 1 && !nameExists &&
                            (
                ...
                <Flex mt={5} justifyContent="end">
                    <Button
                        ...
                        ml={3}
                        isDisabled={
                            isCheckingEmail || isCheckingName || !isValidEmail ||
                            !firstName || !lastName || !email || !emailsMatch || emailExists || nameExists
                        }
                    >
                        Add User
                    </Button>
