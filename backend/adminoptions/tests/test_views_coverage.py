...
- SendTestEmail and SendAllTestEmails
...
class TestSendTestEmail:
    """Tests for SendTestEmail view

    Covers lines ~700-800 (email building, CID attachment, preview saving).
    """

    def test_send_test_email_no_admin_options(self, api_client, admin_user, db):
        """POST without admin options returns 400"""
        api_client.force_authenticate(user=admin_user)
        response = api_client.post(adminoptions_urls.path("send-test-email"))
        assert response.status_code == status.HTTP_400_BAD_REQUEST
        assert "Email testing mode" in response.data["error"]

    def test_send_test_email_no_test_user(self, api_client, admin_user, db):
    ...
    def test_send_test_email_success(
        ...
        """POST sends test email successfully"""
        api_client.force_authenticate(user=admin_user)
    ...
    def test_send_test_email_failure(
        ...
        """POST handles email send failure"""
        api_client.force_authenticate(user=admin_user)
        admin_user.display_first_name = "Admin"
        admin_user.display_last_name = "User"
        admin_user.save()
    ...
    def test_send_test_email_requires_admin(self, api_client, user, db):
        """Regular user cannot send test emails"""
        api_client.force_authenticate(user=user)
        response = api_client.post(adminoptions_urls.path("send-test-email"))
        assert response.status_code == status.HTTP_403_FORBIDDEN
