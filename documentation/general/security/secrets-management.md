...
Redis is **not currently deployed** in Kubernetes configuration. The application is configured to use Redis if available, but gracefully degrades to a dummy cache if Redis is not present.

### Is There a Need for Redis?

**Yes, plan to deploy Redis** because:

1. Users have increased to 1000+ users, some with public staff profiles
...
This is the simplest approach - add Redis as a sidecar container in the existing deployment.
...
**For Staging/Test**: Use **Option 1** (sidecar pattern)
...
- Update production secrets without testing in staging
...
- **Kubernetes Overview**: `../deployment/kubernetes-overview.md` - Deployment configuration
- **Backend Local Setup**: `../../backend/development/local-setup.md` - Local environment variables
- **Backend ADR-008**: `../../backend/architecture/ADR-008-redis-application-caching.md` - Redis caching strategy
